...
public:
	...
	void fillStack(); 
	//Populate stack with random numbers. 
	//Uses MaxStackSize to generate proper number of elements.
...
bool stackType<Type>::operator==(const stackType<Type>& otherStack) const
{
	...
	//Check elements for equality relative to position
...
template <class Type>
void stackType<Type>::fillStack()
{
	int count;
	int num;

	for (count = 0; count < maxStackSize; count++)
	{
		num = (rand());
		push(num);
	}
